...
6. Next logical step: browser-side lifecycle smoke for archive/restore/final delete, then live-test driven UX/retrieval hardening. Reimport semantics and bulk/folder import remain later refinements.
7. Keep the current strict workbench workflows stable while planning the knowledge-base slice: `search_findings`, full-document person seeds, research-finding conversion, source validation, and contradiction detection should remain auditable and source-bound.
8. Keep the dedicated `Audit napló` backend/API/panel over `audit_events` as the following larger work surface after the knowledge-base module. It should remain separate from `Elemzési előzmények`, which is `analysis_runs`-based.
9. Treat the implemented `iratgyujtemeny` backend/frontend slice as the stable first source-scope layer. Add multi-collection source-scope controls only if a concrete analysis/RAG/knowledge-base workflow needs them; the current first integration intentionally uses one selected collection at a time.
10. Keep the current document-isolated generation strategy for RAG and `search_findings`: retrieval is relevance-based for source selection, but LLM input is ordered by document/page/chunk. `search_findings` batches never mix documents; multi-document RAG asks for per-document partial answers and then a final synthesis.
11. Treat a serious `Jogszabályi kereső` as a later specialized module, not a small label change inside the generic RAG module. It will need law/section/paragraph/effective-date semantics and a separate design pass.
12. Continue opportunistic live-test driven fixes for the implemented `Általános iratkérdező`, full-document person profile, desktop/1080p/mobile UI, and prompt/JSON fallback paths, but do not treat them as the main planned slice unless a concrete bug or quality issue appears.
