...
- Current verification baseline: latest known full suite `pytest: 377 passed`, latest targeted document-discard regression slice `59 passed`, latest targeted knowledge query/API slice `52 passed`, `alembic: 0047_knowledge_index_metadata (head)`, `npm --prefix frontend run build` passed.
...
- Current RAG defaults are `retrieval_strategy=hybrid`, `max_chunks=30`, max `60`. Multi-document RAG is document-isolated: retrieval selects relevant chunks, chunks are reordered by document/page/chunk, each contributing document gets its own partial LLM answer, and a final synthesis answer is generated from those partial answers. Single-document RAG still uses one direct answer call.
...
- JSON handling now has schema-specific fallback recovery for malformed local-model JSON caused by unescaped internal double quotes. `search_findings` can recover its ordered `findings` shape, and full-document person processing can recover its ordered `items` shape (`item_kind`, `display_label`, `recommended_search_focus`, `source_label`). These fallbacks do not bypass normal source validation.
...
2. Continue the global `Tudásbázis` slice from `Design_documents/21_markdown_knowledge_base_module_plan.md`. Implemented so far: migration `0046_knowledge_documents`, `KnowledgeDocumentModel`, knowledge Pydantic schemas, deterministic `app/services/markdown_parser.py`, `app/services/knowledge_import.py`, data-root/text-store backed `.md` persistence/chunk-manifest writing, migration `0047_knowledge_index_metadata`, `app/services/knowledge_indexing.py`, separate knowledge Qdrant collection naming, `GET /api/v1/knowledge/index/status`, `POST /api/v1/knowledge/index`, `app/services/knowledge_query.py`, `POST /api/v1/knowledge/query`, first archive/restore/final-delete lifecycle endpoints, batch-only Markdown import endpoints, router registration, and focused parser/import/API/index/query/lifecycle tests.
3. Live import -> index -> query smoke passed with a generated Markdown document: import created one `markdown_note` with 2 chunks, knowledge indexing wrote 2 chunks into `boberdetective_chunks_knowledge_text_embedding_bge_m3`, and a hybrid short query returned a source-backed answer with source cards.
4. First `Tudásbázis` frontend surface is implemented: sidebar navigation entry, Markdown import form, knowledge document list/search/selection, selected-document toolbar actions, index status/index action, query input, and current answer/source cards. The module should not be content-specialized; IT security notes, legal notes, technical documentation, and other structured Markdown corpora should all fit the same shape.
5. User-side live test of the first `Tudásbázis` frontend flow passed: Markdown import, indexelés, kérdezés, válasz/forrás megjelenítés, és dokumentumrészlet nézet alapvetően működik. First lifecycle workflow is now implemented too: archive, restore, final delete, data-root cleanup on delete, Qdrant knowledge-point cleanup, global audit events, and frontend controls.
...
8. Same-hash and same-relative-path replacement behavior is working with the relative-directory-plus-filename import key. The former single-file conflict-decision UI has been retired with the public one-file import endpoint/UI; the batch preview/import UI now owns all import conflict decisions.
...
14. User-side live testing confirmed the batch import behavior and current knowledge-base UI are usable. The Marko/AST Markdown parser from `Design_documents/22_markdown_ast_chunking_plan.md` is now the active parser baseline in `app/services/markdown_parser.py`; the old line-based implementation and comparison script were removed after live testing on 121 Markdown documents / 2708 chunks. Future parser changes should be driven by concrete bad chunk examples rather than broad retuning.
15. Knowledge-base retrieval hardening now includes Markdown-aware scoring, section expansion, document-level packing, heading-meta bridging, and the accepted section `12d` context-packing tuning from `Design_documents/24_markdown_section_aware_retrieval_packing_plan.md`. User-side live validation on the larger Markdown corpus showed a clear quality improvement, so this is the current stable `Tudásbázis` retrieval baseline. Future tuning should be driven only by concrete bad source-selection examples rather than broad parser retuning or backend query rewriting.
16. Keep the current strict workbench workflows stable while planning the knowledge-base slice: `search_findings`, full-document person seeds, research-finding conversion, source validation, and contradiction detection should remain auditable and source-bound.
17. Keep the dedicated `Audit napló` backend/API/panel over `audit_events` as the following larger work surface after the knowledge-base module. It should remain separate from `Elemzési előzmények`, which is `analysis_runs`-based.
18. Treat the implemented `iratgyujtemeny` backend/frontend slice as the stable first source-scope layer. Add multi-collection source-scope controls only if a concrete analysis/RAG/knowledge-base workflow needs them; the current first integration intentionally uses one selected collection at a time.
19. Keep the current document-isolated generation strategy for RAG and `search_findings`: retrieval is relevance-based for source selection, but LLM input is ordered by document/page/chunk. `search_findings` batches never mix documents; multi-document RAG asks for per-document partial answers and then a final synthesis.
20. Treat a serious `Jogszabályi kereső` as a later specialized module, not a small label change inside the generic RAG module. It will need law/section/paragraph/effective-date semantics and a separate design pass.
21. Continue opportunistic live-test driven fixes for the implemented `Általános iratkérdező`, full-document person profile, desktop/1080p/mobile UI, and prompt/JSON fallback paths, but do not treat them as the main planned slice unless a concrete bug or quality issue appears.
...
- The previous major product directions, the `Általános iratkérdező` and the dedicated `Tudásbázis` module, both have stable implementation baselines. The `Tudásbázis` module for Markdown-based structured knowledge material is planned in `Design_documents/21_markdown_knowledge_base_module_plan.md`; its import/chunking/indexing/query backend foundation, frontend surface, lifecycle workflow, relative-directory-plus-filename import key semantics, batch/folder Markdown import contract, batch-preview/import endpoints, Marko/AST parser, safe Markdown answer/source rendering, and Markdown-aware retrieval hardening layers are implemented/documented. The public single-file import API/UI is retired; the batch UI is the only import surface and also covers one-file imports. It requires a relative directory, shows only conflict/invalid item rows, and asks for user decisions only on same-path/different-content conflicts. The current Tudásbázis retrieval state is the accepted stable baseline after live validation; further work here should be concrete-example-driven hardening. The dedicated `Audit napló` API/panel over `audit_events` is now the next larger planned work surface, while RAG/full-document/UI work should continue as live-test driven hardening rather than broad new implementation.
...
- Latest test run: `377 passed`, 1 Docling deprecation warning.
