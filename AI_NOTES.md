...
- Switched the active chat-model default to `qwen/qwen3.6-35b-a3b` for quality testing with `context_length=61440`, `eval_batch_size=512`, `flash_attention=true`, and `offload_kv_cache_to_gpu=true`. The previous speed-optimized fallback remains documented as `qwen/qwen3.5-9b` with `eval_batch_size=4096` and the same context/attention/KV settings. Live backend load accepted this profile: `status=loaded`, `load_time_seconds=18.773`.
...
- Current preferred chat-model LM Studio load profile is configured for quality testing as `qwen/qwen3.6-35b-a3b` with `context_length=61440`, `eval_batch_size=512`, `flash_attention=true`, and `offload_kv_cache_to_gpu=true`. Previous speed-optimized fallback: `qwen/qwen3.5-9b` with `eval_batch_size=4096` and the same context/attention/KV settings.
...
- The previous Qwen embedding profile is intentionally retired and should not be restored as the default. The current quality-test two-model profile uses chat `qwen/qwen3.6-35b-a3b`, chat `context_length=61440`, chat `eval_batch_size=512`, BGE-M3 embedding `context_length=4096`, and LLM request timeout `900` seconds for long full-document runs. The previous balanced/speed fallback remains `qwen/qwen3.5-9b` with `eval_batch_size=4096`.
