...
- Current verification baseline: `pytest: 304 passed`, `alembic: 0045_limit_rag_answer_modes (head)`, `npm --prefix frontend run build` passed.
...
- Current RAG defaults are `retrieval_strategy=hybrid`, `max_chunks=45`, max `90`. Multi-document RAG is document-isolated: retrieval selects relevant chunks, chunks are reordered by document/page/chunk, each contributing document gets its own partial LLM answer, and a final synthesis answer is generated from those partial answers. Single-document RAG still uses one direct answer call.
...
- A dedicated mobile media query now starts at `max-width: 760px` and should remain the place for phone-specific layout behavior. It stacks work-surface grids, moves the sidebar into normal flow, gives input/button rows separate lines, resets desktop dashboard heights where they hurt mobile reading, fixes long source/document text and chip overflow, and uses a mobile-specific `Ügy munkapad` panel order. Keep future phone tweaks in this block unless a problem also exists on desktop.
...
2. Continue the global `Tudásbázis` slice from `Design_documents/21_markdown_knowledge_base_module_plan.md`. Implemented so far: migration `0046_knowledge_documents`, `KnowledgeDocumentModel`, knowledge Pydantic schemas, deterministic `app/services/markdown_parser.py`, `app/services/knowledge_import.py`, data-root/text-store backed `.md` persistence/chunk-manifest writing, `/api/v1/knowledge/documents` import/list/detail endpoints, migration `0047_knowledge_index_metadata`, `app/services/knowledge_indexing.py`, separate knowledge Qdrant collection naming, `GET /api/v1/knowledge/index/status`, `POST /api/v1/knowledge/index`, `app/services/knowledge_query.py`, `POST /api/v1/knowledge/query`, first archive/restore/final-delete lifecycle endpoints, router registration, and focused parser/import/API/index/query/lifecycle tests.
...
4. First `Tudásbázis` frontend surface is implemented: sidebar navigation entry, Markdown import form, knowledge document list/search/selection, index status/index action, query input, current answer/source cards, and document detail/chunk preview panel. The module should not be content-specialized; IT security notes, legal notes, technical documentation, and other structured Markdown corpora should all fit the same shape.
...
6. First backend import-conflict contract is implemented for knowledge-base Markdown imports: `conflict_strategy = fail | skip | replace`, same `sha256_hash` can fail or skip with warning and no mutation, same stored `relative_path` with different hash can replace only through explicit `replace`, and replacement creates the new document before deleting the old data-root/Qdrant/DB record. This contract is batch-import friendly from the start.
...
8. One-file frontend conflict decision UI is implemented and user-side live-smoked. On structured 409 conflict, the import panel shows `Meglévő megtartása` / `Csere az új fájlra`; same-hash conflicts are shown as already imported and do not offer replacement. Same-hash and same-relative-path replacement behavior is working with the relative-directory-plus-filename import key.
9. Next logical step: prepare the batch/folder Markdown import slice, reusing the same conflict contract and extending it with an "alkalmazd az összes ilyen ütközésre" style decision model.
10. Keep the current strict workbench workflows stable while planning the knowledge-base slice: `search_findings`, full-document person seeds, research-finding conversion, source validation, and contradiction detection should remain auditable and source-bound.
11. Keep the dedicated `Audit napló` backend/API/panel over `audit_events` as the following larger work surface after the knowledge-base module. It should remain separate from `Elemzési előzmények`, which is `analysis_runs`-based.
12. Treat the implemented `iratgyujtemeny` backend/frontend slice as the stable first source-scope layer. Add multi-collection source-scope controls only if a concrete analysis/RAG/knowledge-base workflow needs them; the current first integration intentionally uses one selected collection at a time.
13. Keep the current document-isolated generation strategy for RAG and `search_findings`: retrieval is relevance-based for source selection, but LLM input is ordered by document/page/chunk. `search_findings` batches never mix documents; multi-document RAG asks for per-document partial answers and then a final synthesis.
14. Treat a serious `Jogszabályi kereső` as a later specialized module, not a small label change inside the generic RAG module. It will need law/section/paragraph/effective-date semantics and a separate design pass.
15. Continue opportunistic live-test driven fixes for the implemented `Általános iratkérdező`, full-document person profile, desktop/1080p/mobile UI, and prompt/JSON fallback paths, but do not treat them as the main planned slice unless a concrete bug or quality issue appears.
...
- Latest test run: `338 passed`, 1 Docling deprecation warning.
