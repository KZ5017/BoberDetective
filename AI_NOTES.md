...
2. Continue the global `Tudásbázis` slice from `Design_documents/21_markdown_knowledge_base_module_plan.md`. Implemented so far: migration `0046_knowledge_documents`, `KnowledgeDocumentModel`, knowledge Pydantic schemas, deterministic `app/services/markdown_parser.py`, `app/services/knowledge_import.py`, data-root/text-store backed `.md` persistence/chunk-manifest writing, `/api/v1/knowledge/documents` import/list/detail endpoints, migration `0047_knowledge_index_metadata`, `app/services/knowledge_indexing.py`, separate knowledge Qdrant collection naming, `GET /api/v1/knowledge/index/status`, `POST /api/v1/knowledge/index`, `app/services/knowledge_query.py`, `POST /api/v1/knowledge/query`, router registration, and focused parser/import/API/index/query tests.
...
5. User-side live test of the first `Tudásbázis` frontend flow passed: Markdown import, indexelés, kérdezés, válasz/forrás megjelenítés, és dokumentumrészlet nézet alapvetően működik. Remaining first hardening target: knowledge document lifecycle workflow, including real delete/archive/reimport semantics, data-root cleanup, Qdrant knowledge-point cleanup, and audit events.
6. Keep the current strict workbench workflows stable while planning the knowledge-base slice: `search_findings`, full-document person seeds, research-finding conversion, source validation, and contradiction detection should remain auditable and source-bound.
7. Keep the dedicated `Audit napló` backend/API/panel over `audit_events` as the following larger work surface after the knowledge-base module. It should remain separate from `Elemzési előzmények`, which is `analysis_runs`-based.
8. Treat the implemented `iratgyujtemeny` backend/frontend slice as the stable first source-scope layer. Add multi-collection source-scope controls only if a concrete analysis/RAG/knowledge-base workflow needs them; the current first integration intentionally uses one selected collection at a time.
9. Keep the current document-isolated generation strategy for RAG and `search_findings`: retrieval is relevance-based for source selection, but LLM input is ordered by document/page/chunk. `search_findings` batches never mix documents; multi-document RAG asks for per-document partial answers and then a final synthesis.
10. Treat a serious `Jogszabályi kereső` as a later specialized module, not a small label change inside the generic RAG module. It will need law/section/paragraph/effective-date semantics and a separate design pass.
11. Continue opportunistic live-test driven fixes for the implemented `Általános iratkérdező`, full-document person profile, desktop/1080p/mobile UI, and prompt/JSON fallback paths, but do not treat them as the main planned slice unless a concrete bug or quality issue appears.
