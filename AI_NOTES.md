...
6. Continue live-testing and hardening the implemented `Általános iratkérdező` work surface. It now has backend/frontend/API persistence through migrations `0044_rag_answers` and `0045_limit_rag_answer_modes`, `rag_query` provenance, temporary answer display, explicit save/list/detail/delete, default hybrid retrieval, selected-document case scopes, latest-run summary, saved `source_summary`, and only two answer modes: `short` and `detailed`. Do not restore `source_focused` or `strict_source` unless a better model/prompt design proves they help.
7. Keep the current document-isolated generation strategy for RAG and `search_findings`: retrieval is relevance-based for source selection, but LLM input is ordered by document/page/chunk. `search_findings` batches never mix documents; multi-document RAG asks for per-document partial answers and then a final synthesis. This is slower but reduces cross-document context blending.
