...
- Current verification baseline: latest known full suite pytest: 409 passed, latest targeted document-discard regression slice 59 passed, latest targeted knowledge query/API slice 52 passed, latest targeted relationship graph backend slice 9 passed, latest targeted full-document/documents/RAG slice 85 passed, latest full-document unit slice 22 passed, latest targeted AI-asszisztens/LLM slice 28 passed, alembic: 0051_assistant_chats (head), npm --prefix frontend run build passed.

Latest session update:

- Added local LM Studio authentication support through the Git-ignored root `.env`, without overriding exported environment variables; the existing provider clients use the configured API key as a Bearer token. BoberDetective now starts independently beside AI Assistant through tracked `scripts/start.ps1` / `scripts/stop.ps1`, using backend port `8001` and frontend port `5174`.

- Implemented `Design_documents/37_research_finding_attach_existing_object_plan.md`: `Kutatási találatok` cards now support attaching a source-valid research finding's existing `source_reference_id` directly to an already-created structured object. The backend exposes `POST /api/v1/cases/{case_id}/research-findings/{finding_id}/attach-source`, marks the finding `converted`, stores `target_object_type` / `target_object_id`, keeps the finding metadata as provenance only, and avoids only exact duplicate `source_reference_id + target object` links. Frontend cards now have a `Meglévő találathoz csatolás` section with type and searchable target selection. Latest focused verification: `tests/test_research_findings.py` 17 passed, `tests/test_research_findings.py tests/test_manual_entries.py` 22 passed, `npm --prefix frontend run build` passed, `git diff --check` passed.

- Restored the active chat-model default to the speed-optimized `qwen/qwen3.5-9b` profile with `context_length=61440`, `eval_batch_size=4096`, `flash_attention=true`, and `offload_kv_cache_to_gpu=true`. The larger `qwen/qwen3.6-35b-a3b` quality profile remains a documented local opt-in with `eval_batch_size=512`.
...
- Small UI stability fixes are in place: Export előzmények compact cards no longer stretch vertically to fill the panel, and Kapcsolati térkép object cards in Megjelenítendő objektum and Kapcsolódó objektumok use content-driven height instead of the old graph-object fixed minimum. Latest frontend verification for this slice: npm --prefix frontend run build passed.
...
- `Design_documents/32_relationship_map_related_objects_plan.md` is now implemented as the current graph related-object slice: the old checkbox/layer-based `Kapcsolódó objektumok` shared-source behavior was removed from frontend and backend, and replaced by an explicit related-by-document workflow. In that workflow the selected object means the currently clicked graph object node, results appear in the dedicated `Kapcsolódó objektumok` panel directly after `Megjelenítendő objektum`, the backend query is `POST /api/v1/cases/{case_id}/graph/related-by-documents`, `Keresés` and `Láthatók kijelölése` drive the result list, and only user-selected related results are added to the map under the existing 50 focus-object cap. Graph object cards now use object-type color roles instead of rendering type/shared-document metadata as a third line.
- The `Kapcsolati térkép` technical multi-focus baseline is implemented and can be treated as the current functional state. The graph projection uses the multi-focus POST API: `RelationshipGraphMultiFocusRequest`, `focus_node_ids` / `focus_objects` response metadata, shared `build_relationship_graph_for_objects(...)`, single-focus builder delegation, and `POST /api/v1/cases/{case_id}/graph/objects`. It supports source-valid `claim`, `event`, `entity`, `missing_item_candidate`, and `contradiction_candidate` focus objects, deterministic `document -> page -> chunk -> source_reference -> object` source-location provenance with fallbacks, and contradiction claim-pair edges. Shared-source neighbor projection and `SHARES_SOURCE_WITH` are retired from the normal object-centered graph. The graph no longer uses node/edge truncation as the normal guardrail; instead, frontend and backend both enforce a 50 focus-object cap. The object-centered graph intentionally no longer displays `Elemzési eredet`: analysis-run and research-finding lifecycle provenance remains available through the existing analysis/audit surfaces, not as graph nodes. The frontend surface is implemented with source-valid checkbox object-card selection, `x kijelölve / 50` plus `Összes: n`, `Térkép frissítése kijelölésből`, `Láthatók kijelölése`, `Láthatók levétele`, `Térkép ürítése`, `Ügy munkapad` handoff through a one-object POST request, lazy-loaded React Flow / XYFlow canvas, temporary node dragging without persistence, the current two-row inspector layout, explicit frontend graph layer toggles, and the top-row `Kapcsolódó objektumok` panel placed immediately after the object selector. Layer order is `Irat`, `Oldal`, `Szövegrész`, `Forráshivatkozás`, `Ellentmondások`; `Irat` and `Forráshivatkozás` are enabled by default. The old UUID-shaped single-focus GET endpoint and frontend helper have been retired. The relationship-map object selector uses its own source-valid review-report data set, so `Áttekintési jelentés` panel filters no longer leak into `Megjelenítendő objektum`. Next graph work is live UX/readability strengthening of this baseline, not core multi-focus or related-object plumbing.
...
- Latest relationship-graph backend verification after the focus-limit/object-selector cleanup: `.venv/bin/python -m pytest tests/test_relationship_graph.py -q` returned `8 passed`.
...
- The Git-ignored root `.env` is loaded as local development configuration without overriding exported environment variables. Use it for the LM Studio Bearer API key when authentication is enabled; never commit the real token.
...
- Current preferred chat-model LM Studio load profile is `qwen/qwen3.5-9b` with `context_length=61440`, `eval_batch_size=4096`, `flash_attention=true`, and `offload_kv_cache_to_gpu=true`. The quality profile `qwen/qwen3.6-35b-a3b` remains an explicit local opt-in with `eval_batch_size=512`.
...
- The previous Qwen embedding profile is intentionally retired and should not be restored as the default. The current workstation profile uses chat `qwen/qwen3.5-9b`, chat `context_length=61440`, chat `eval_batch_size=4096`, BGE-M3 embedding `context_length=4096`, and LLM request timeout `900` seconds for long full-document runs. The quality opt-in remains `qwen/qwen3.6-35b-a3b` with `eval_batch_size=512`.
...
- Latest test run: `409 passed`, 1 Docling deprecation warning.
