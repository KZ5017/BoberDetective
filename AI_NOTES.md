...
- Current verification baseline: latest known full suite `pytest: 349 passed`, latest targeted document-discard regression slice `59 passed`, latest targeted knowledge query/API slice `22 passed`, `alembic: 0047_knowledge_index_metadata (head)`, `npm --prefix frontend run build` passed.
...
- The next planned `Tudásbázis` quality slice is retrieval hardening rather than more parser retuning. Inspect and improve how semantic/hybrid retrieval selects chunks before the LLM sees them: query variants, Markdown-aware hybrid scoring, heading/code/token boosts, optional neighboring chunk expansion within the same document/heading, document/heading/chunk ordering, and a deterministic reranking layer if needed. Keep the keyword mode as the simple baseline.
...
15. Keep the current strict workbench workflows stable while planning the knowledge-base slice: `search_findings`, full-document person seeds, research-finding conversion, source validation, and contradiction detection should remain auditable and source-bound.
16. Keep the dedicated `Audit napló` backend/API/panel over `audit_events` as the following larger work surface after the knowledge-base module. It should remain separate from `Elemzési előzmények`, which is `analysis_runs`-based.
17. Treat the implemented `iratgyujtemeny` backend/frontend slice as the stable first source-scope layer. Add multi-collection source-scope controls only if a concrete analysis/RAG/knowledge-base workflow needs them; the current first integration intentionally uses one selected collection at a time.
18. Keep the current document-isolated generation strategy for RAG and `search_findings`: retrieval is relevance-based for source selection, but LLM input is ordered by document/page/chunk. `search_findings` batches never mix documents; multi-document RAG asks for per-document partial answers and then a final synthesis.
19. Treat a serious `Jogszabályi kereső` as a later specialized module, not a small label change inside the generic RAG module. It will need law/section/paragraph/effective-date semantics and a separate design pass.
20. Continue opportunistic live-test driven fixes for the implemented `Általános iratkérdező`, full-document person profile, desktop/1080p/mobile UI, and prompt/JSON fallback paths, but do not treat them as the main planned slice unless a concrete bug or quality issue appears.
...
- Latest test run: `349 passed`, 1 Docling deprecation warning.
