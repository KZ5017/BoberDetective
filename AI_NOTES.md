...
- Current verification baseline: latest known full suite `pytest: 349 passed`, latest targeted document-discard regression slice `59 passed`, `alembic: 0047_knowledge_index_metadata (head)`, `npm --prefix frontend run build` passed.
...
14. User-side live testing confirmed the batch import behavior and current knowledge-base UI are usable. Next knowledge-base work should be incremental hardening: larger-corpus import/index smoke, optional folder-selection enhancement if browser-provided relative paths are useful enough, and small UX fixes discovered during real use.
