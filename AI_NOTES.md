...
- Current verification baseline: latest known full suite `pytest: 384 passed`, latest targeted document-discard regression slice `59 passed`, latest targeted knowledge query/API slice `52 passed`, `alembic: 0048_review_status_cleanup (head)`, `npm --prefix frontend run build` passed.
...
- The next larger planned development slice is now the `Kapcsolati térkép` direction captured in `Design_documents/25_relationship_map_graph_view_plan.md`: a dedicated, read-only, object-centered graph work surface over existing source-valid structured objects, backed by a backend graph-projection API and React Flow / XYFlow in the frontend. This should be the next design/implementation direction unless the user reprioritizes.
...
4. Make `Design_documents/25_relationship_map_graph_view_plan.md` the next larger planned slice: refine and implement a dedicated `Kapcsolati térkép` work surface.
...
7. Keep the dedicated `Audit napló` backend/API/panel over `audit_events` as a later larger work surface unless explicitly reprioritized.
8. Continue opportunistic live-test driven fixes for implemented `Általános iratkérdező`, `Tudásbázis`, full-document person profile, desktop/1080p/mobile UI, and prompt/JSON fallback paths, but do not treat them as the main planned slice unless a concrete bug or quality issue appears.
...
- Latest test run: `384 passed`, 1 Docling deprecation warning.
