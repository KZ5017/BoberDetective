...
- Added `Design_documents/27_relationship_map_graph_implementation_plan.md` with the concrete first implementation plan for `Kapcsolati térkép`: one source-valid focus object, explicit node/edge model, graph projection API, React Flow / XYFlow frontend surface, `Ügy munkapad` handoff, limits, and test scope.
- Implemented the first backend `Kapcsolati térkép` graph-projection slice: `GET /api/v1/cases/{case_id}/graph/object/{object_type}/{object_id}` returns a read-only node/edge projection for one source-valid focus object, including direct source references, document/page/chunk location nodes, analysis-run provenance, research-finding conversion provenance, contradiction claim-pair links, shared-source neighbors, limits, and warnings.
- Added the first frontend `Kapcsolati térkép` skeleton: sidebar work surface, graph API types/helper, source-valid object selection path, empty/loading/error integration through the shared surface feedback flow, and a token-aligned node/edge preview panel. The early manual UUID input was replaced by the object-card selector in the completed baseline.
- Added the first read-only React Flow / XYFlow visualization for `Kapcsolati térkép`: graph nodes are deterministically layered by provenance, focus object, source reference, source location, and related object; pan/zoom/fit controls are enabled, editing/connecting/dragging is disabled, and the existing node/edge preview remains below the canvas for inspectability.
- Reworked the `Kapcsolati térkép` frontend layout into the accepted first inspector workbench shape: top row `Megjelenítendő objektum`, `Elemek`, and `Kapcsolatok` panels, with a full-width React Flow canvas below. Node and edge clicks now drive the inspector panels instead of relying on a long always-visible node/edge list.
- Implemented and documented the `Kapcsolati térkép` content-UX slice: frontend layer toggles keep the default graph focused on the object, source reference, and source context, while detailed `Szövegrész`, `Elemzési eredet`, `Kapcsolódó objektumok`, and `Ellentmondások` are optional checkbox layers over the existing backend graph response.
- Updated the graph implementation plan and handoff docs so the current fixed next slice is multi-focus graph support: a `POST /graph/objects` style endpoint, multiple source-valid object selection, later whole-case graph only if stable, and mobile fine-tuning through live review. Full backend test-matrix expansion, edge filters, layout persistence, audit nodes, and human review nodes are recorded as conscious later backlog.
- Expanded the multi-focus graph target description before implementation planning: the next graph behavior should support up to 20 source-valid focus objects, checkbox selection, shared deduplicated node/edge output, preserved graph layer toggles, single-focus handoff compatibility, and later retirement of the current single-focus GET endpoint after frontend migration.
- Added the concrete multi-focus graph implementation plan: backend request/response schema additions, shared `build_relationship_graph_for_objects(...)` service path, `POST /cases/{case_id}/graph/objects`, frontend POST helper, checkbox selection state, handoff migration, rendering updates for multiple focus nodes, targeted tests, implementation order, and risk controls.
- Implemented the first backend multi-focus graph slice: graph schemas now include focus-object request models and multi-focus response metadata, the single-focus builder delegates to shared `build_relationship_graph_for_objects(...)`, `POST /api/v1/cases/{case_id}/graph/objects` returns a deduplicated graph for 1..20 source-valid focus objects, and targeted relationship-graph tests now cover single-focus compatibility, shared-source deduplication, source-invalid rejection, and focus cap behavior.
- Implemented the frontend multi-focus graph migration: graph loading now uses the POST endpoint for both normal selection and one-object handoff, object cards use checkbox selection with a `x kijelölve / 20` counter, selected objects can be cleared, the graph action is `Kijelöltek térképre helyezése`, focus nodes remain visible through layer filtering, and the old UUID-shaped single-focus GET route/helper was retired.
- Refined the `Kapcsolati térkép` object-selection and layer UX: the object type selector now has `Összes`, the current filtered list can be selected or removed in bulk, the displayed graph can be refreshed from the current selection or emptied, and the layer toggles now explicitly include `Irat`, `Oldal`, `Szövegrész`, `Forráshivatkozás`, `Elemzési eredet`, `Kapcsolódó objektumok`, and `Ellentmondások`, with `Irat` plus `Forráshivatkozás` enabled by default.
- Added frontend-only visual bridge edges for `Kapcsolati térkép`: when `Forráshivatkozás` is hidden but source context remains visible, the canvas draws label-free dashed `VISUAL_SOURCE_BRIDGE` links from document/page/chunk nodes to focus objects without changing backend graph truth.
- Documented the next `Kapcsolati térkép` backend projection cleanup: replace the source-reference-centered source-location star with a backend-projected hierarchy `document -> page -> chunk -> source_reference -> object`, with fallbacks when page or chunk data is unavailable.
- Documented the current `Kapcsolati térkép` state as a technically complete multi-focus baseline; the next graph work is visual/readability strengthening and live UX tuning rather than missing core plumbing.
- Added `Design_documents/28_relationship_map_horizontal_layout_plan.md` to define the next graph visual slice: a deterministic horizontal layer layout that arranges source/document/chunk nodes, source-reference nodes, focus objects, related objects, and optional provenance in stable workbench-style columns.
- Implemented the horizontal graph layout helper for `Kapcsolati térkép`: React Flow nodes are now placed in deterministic document -> page -> chunk -> source-reference -> object -> contradiction layers, provenance nodes use a separate lower lane, all multi-focus `focus_node_ids` are highlighted, and node visual roles are slightly differentiated through existing CSS tokens.
- Cleaned up the frontend build/tooling warnings introduced around the graph canvas work: `@xyflow/react` is lazy-loaded through a separate `RelationshipFlowCanvas` chunk, `esbuild` is pinned through npm `overrides` to `0.28.1` so `npm audit --audit-level=high` reports no vulnerabilities on the current Node 18 runtime, and Vite build target/warning limits are set for the modern local-app deployment profile.
- Implemented the `Kapcsolati térkép` backend source-location hierarchy projection: source references now expose the natural graph chain `document -> page -> chunk -> source_reference -> object`, with deterministic fallbacks for missing page/chunk data. The frontend graph layers were split into `Irat`, `Oldal`, `Szövegrész`, and `Forráshivatkozás`, with visual-only bridge edges when intermediate layers are hidden.
- Reoriented `Kapcsolati térkép` contradiction candidate graph semantics: `CONTRADICTS_CLAIM_A/B` edges now run from claim/object to contradiction candidate, and the React Flow layout places contradiction candidates in a right-side object-derived column while keeping their visual style object-like.
- Simplified contradiction candidate rendering in `Kapcsolati térkép`: contradiction candidates no longer add their own direct source-reference graph legs, so the map reads through the two input claims and avoids duplicate source lines.
...
- Implemented manual contradiction-candidate cleanup through migration `0049_contradiction_partial_pair`: corrected contradiction candidates may retain partial/missing pairs, the detail panel can detach the `A` or `B` claim side, detach actions are audited as corrections, and claim/event deletion now marks dependent contradiction candidates `Korrekcióval kizárt` instead of deleting them as a hidden side effect.
- Restored the broader `Szövegrész plafon` defaults for investigative RAG/finding workflows while keeping the knowledge-base query tighter: `Elemzés` and `Kérdés az iratállományhoz` now default to `45` and cap at `90`; `Kérdés a tudásbázishoz` remains `30/60`.
- Changed `Teljes iratfeldolgozás` person-search source evidence so worklist cards show the full source sentence containing the matched person name, generated by backend sentence-span expansion, instead of repeating only the bare name.
- Added `Design_documents/29_full_document_free_question_plan.md` for the planned `Szabad iratkérdés` full-document profile: selected active document pages plus a user question should produce persistent `full_document_answers` and an `Iratválasz` style UI, separate from the person-only `document_processing_items` worklist.
- Implemented the first `Szabad iratkérdés` slice: migration `0050_full_document_answers`, `FullDocumentAnswerModel`, `full_document_answer` analysis-run output support, profile-aware full-document run branching, answer list/detail/delete APIs, and a frontend `Iratválasz` panel with one-line question input, safe Markdown answer rendering, saved-answer switching, and answer deletion. Follow-up hardening refreshes the saved-answer list after deletion, selects the next available answer when possible, applies the shared tokenized choice-button style to previous-answer selectors, and recovers usable free-question answers when optional JSON metadata is malformed, internal quotes are unescaped, or the final object brace is missing while `answer_text` is present.

### Verified

- `.venv/bin/python -m pytest` (`392 passed`, 1 Docling deprecation warning)
- `.venv/bin/python -m pytest tests/test_full_document_processing.py tests/test_documents.py tests/test_rag.py -q` (`85 passed`, 1 Docling deprecation warning)
- `.venv/bin/python -m pytest tests/test_full_document_processing.py -q` (`22 passed`)
- `npm --prefix frontend run build`
- `.venv/bin/alembic upgrade head` / current head `0050_full_document_answers`
- `.venv/bin/python -m pytest tests/test_health.py tests/test_relationship_graph.py -q` (`10 passed`)
- `npm --prefix frontend run build`
...
- `.venv/bin/python -m pytest tests/test_knowledge_api.py tests/test_knowledge_query.py -q` (`52 passed`)
- `npm --prefix frontend run build`
