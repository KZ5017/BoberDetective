...
```text
pytest full suite, latest known: 349 passed
latest targeted document-discard regression slice: 59 passed
latest targeted knowledge query/API slice: 22 passed
...
- Next logical `Tudásbázis` implementation direction: improve semantic/hybrid source selection quality before further UI polish. Focus on Markdown-aware retrieval and reranking signals such as heading path, code block presence/language, technical token overlap, exact phrase evidence, query variants, same-document/same-heading neighboring chunk expansion, and stable document/heading/chunk ordering before LLM prompting.
