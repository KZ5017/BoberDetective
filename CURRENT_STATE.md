...
- `Design_documents/32_relationship_map_related_objects_plan.md`
- `Design_documents/33_ai_assistant_context_window_plan.md`
- `Design_documents/34_relationship_map_canvas_selection_plan.md`
- `Design_documents/35_general_rag_retrieval_focus_plan.md`
- `Design_documents/36_search_findings_retrieval_focus_plan.md`
- `Design_documents/37_research_finding_attach_existing_object_plan.md`
...
latest targeted AI-asszisztens/LLM slice: 28 passed
...
- The application topbar now uses a compact BoberDetective logo asset beside native UI text. The current shell subtitle is Lokális dokumentum- és tudásintelligencia rendszer, reflecting the broader document, knowledge-base, relationship-map, and AI-assistant scope. PNG asset imports are enabled through frontend/src/vite-env.d.ts.
...
- `Irat rendező` received the latest organizer polish: the document list has a taller stable minimum height, the selected collection view no longer nests an extra `Gyűjtemény tartalma` panel, collection contents scroll inside their own area instead of pushing the whole layout, and the no-selection state uses a normal placeholder rather than auto-selecting the first collection.
...
- The Export előzmények list keeps export cards at their natural height inside the fixed-height export-history panel, preventing the single-card vertical stretch effect.
- `Design_documents/37_research_finding_attach_existing_object_plan.md` is implemented: active `Kutatási találatok` cards can attach a source-valid research finding's existing source reference directly to an already-created structured object. The operation marks the finding `converted`, records `target_object_type` / `target_object_id`, keeps the finding text/relevance/type suggestion as provenance rather than mutating the target object, and avoids only exact duplicate `source_reference_id + target object` links. The frontend block intentionally reuses the same internal `manual-entry-panel` structure as `Új találat forráshivatkozásból -> Meglévő találathoz csatolás`, so spacing and button width stay consistent.
...
- The latest mobile UI pass also covers the knowledge-base and document-organizer surfaces: Markdown import/document-list buttons stack cleanly, long stored Markdown paths wrap inside cards, full-document profile/page controls split into separate rows, full-document worklist buttons use full width, and selected-collection action buttons are full-width in mobile view. Shared choice-button/navigation styling is now tokenized so sidebar-like selectors and previous-answer selectors can use the same hover/active visual language.
- The `Tudásbázis` mobile Markdown answer/source view now has dedicated overflow handling in the mobile media query. Long Markdown paragraphs, links, inline code, fenced code blocks, tables, source metadata, and opened full source chunks stay within their cards/panels through wrapping or local horizontal scrolling where appropriate.
...
- The separate `Általános iratkérdező` work surface is implemented as a local RAG question-answering module over selected case/document/collection source scopes. Design decisions and current contract notes are recorded in `Design_documents/20_general_rag_question_answering_plan.md`: temporary answers are overwritten by the next query, only explicit `Válasz mentése` creates a durable `rag_answers` record, every query keeps `rag_query` `analysis_run` provenance, and the module must not replace strict workbench workflows such as `search_findings` or full-document person seeds. The backend foundation is implemented through migrations `0044_rag_answers` and `0045_limit_rag_answer_modes`, `RagAnswerModel`, `app/schemas/rag.py`, `app/services/rag.py`, `app/api/v1/rag.py`, router registration, and `tests/test_rag.py`. Current `/rag/query` resolves the selected case/document/collection source scope, reuses the existing retrieval foundation, records selected chunks as analysis-run inputs, returns backend-owned `used_sources`, sends labeled source packets to LM Studio, parses the minimal RAG JSON answer, and persists the response summary for explicit `save-answer`. RAG answer modes are intentionally limited to `short` and `detailed`; the experimental `source_focused` / `strict_source` modes were removed from frontend/backend/schema/DB after live tests showed worse hallucinated role attribution under stricter wording. Its current source cap is again `max_chunks` default/cap `45/90`; the tighter `30/60` cap belongs only to the dedicated `Tudásbázis` question surface.
- `Design_documents/35_general_rag_retrieval_focus_plan.md` is implemented as the current `Általános iratkérdező` retrieval-focus refinement. The UI has optional `Keresés fókusza (ajánlott)` above the required `Kérdés`; backend `retrieval_query` is used only for chunk/source selection, falls back to `question` when blank/missing, and is recorded in analysis-run input plus retrieval metadata. LLM prompts and saved-answer question semantics continue to use `question`.
- `Design_documents/36_search_findings_retrieval_focus_plan.md` is implemented for `Ügy munkapad / Kutatási találatok keresése`. The `Elemzés` panel now separates optional `Keresés fókusza (ajánlott)` from required `Elemzési fókusz`: `retrieval_query` selects source chunks only, falls back to `query` when blank/missing, and is persisted in analysis-run input/latest-run metadata. The LLM prompt still receives only the analysis focus, and the backend rejects calls where retrieval focus is present but analysis focus is missing.
- The dedicated `Tudásbázis` work surface is implemented for global Markdown knowledge material. It has a knowledge-only backend model/table, Markdown parser/chunker, data-root text-store persistence, separate knowledge Qdrant indexing, knowledge-only query endpoint, archive/restore/final-delete lifecycle controls, and a frontend surface for import, document selection, indexing, questioning, current answer, and source cards.
- The knowledge-base Markdown parser hardening slice in `Design_documents/22_markdown_ast_chunking_plan.md` is now the active baseline: `app/services/markdown_parser.py` uses Marko/AST parsing (`markdown_marko_ast_parser_v1`, `markdown_ast_sections_v1`) for browser imports, and the old line-based parser implementation plus the old-vs-AST comparison script have been removed. User-side live testing covered 121 Markdown documents / 2708 chunks with good practical results.
- `Tudásbázis` Markdown import is now batch-only. `POST /api/v1/knowledge/documents/batch/preview` previews one or many `.md` files without mutation; `POST /api/v1/knowledge/documents/batch/import` performs partial-success import with explicit replacement only for same-path/different-content conflicts. The public single-file Markdown import endpoint/UI has been retired; one-file import is represented as a one-file batch.
- The `Tudásbázis` desktop UI uses a two-column layout: `Kérdés a tudásbázishoz`, `Tudásanyag importálás`, and `Markdown tudásanyag` on the left, `Aktuális tudásbázis válasz` on the right. Imported Markdown cards show the stored `path + filename` key, not raw hashes; new files do not appear as preview cards, same-hash files are skipped automatically, and only real path conflicts require a visible decision.
...
- The `Tudásbázis` retrieval tuning in `Design_documents/24_markdown_section_aware_retrieval_packing_plan.md` section `12d` is implemented and accepted as the current stable baseline after improved user-side live feedback: hybrid is more keyword-dominant, semantic/hybrid starts from a larger candidate pool, seed/context splitting happens after final scoring, score cutoff applies only to expansion seeds, and high-confidence seeds can pull more forward Markdown context.
...
- Document-level source packing v1 is active for `Tudásbázis` retrieval. Retrieved/expanded candidates are deduplicated by document/chunk, scored per document using the top candidate scores plus a small capped coverage bonus, ordered by document score, and then packed in natural `chunk_index` order inside each document. This keeps related Markdown context together instead of sending a relevance-sorted mixed chunk soup to the LLM.
...
- Current `Általános iratkérdező` answer generation defaults to `hybrid` retrieval, `max_chunks=45`, max `90`, and document-isolated generation when multiple documents contribute selected chunks: retrieval still selects chunks by relevance, selected chunks are reordered by document/page/chunk, each contributing document gets a separate document-level LLM partial answer, and a final synthesis LLM call answers from those partial answers. Single-document RAG still uses one direct LLM call. Responses expose `retrieval_metadata.document_answer_count` so the UI can show how many document partial answers contributed. The separate `Tudásbázis` query surface remains intentionally tighter at `max_chunks` default/cap `30/60`.
...
- `Kapcsolati térkép` backend graph projection is implemented through the multi-focus API. `POST /api/v1/cases/{case_id}/graph/objects` returns a read-only, deduplicated node/edge projection for 1..50 source-valid focus objects. Supported focus types are `claim`, `event`, `entity`, `missing_item_candidate`, and `contradiction_candidate`; the projection can include direct source references, document/page/chunk location nodes, and contradiction claim-pair edges. Shared-source neighbor projection is no longer part of the normal object-centered graph; iratalapú kapcsolódó objektum keresés is handled by the separate `POST /api/v1/cases/{case_id}/graph/related-by-documents` workflow. The graph no longer truncates by node/edge cap during normal projection; `limits` reports the actual returned node/edge counts while the 50 focus-object cap remains the main readability/UX guardrail. It is not a graph database and does not create AI-generated edges. `Elemzési eredet` is intentionally removed from this object-centered graph: analysis-run and research-finding lifecycle provenance remains available through analysis/audit surfaces instead of graph nodes. Chunk-bound source references without explicit `page_id` are resolved to the current document page through the page-local chunk's `page_start` when possible. The old UUID-shaped single-focus GET endpoint has been removed from the public API; service-level single-focus compatibility delegates to the multi-focus builder.
- Contradiction candidate graph edges use the same semantic direction for manual and LLM-created candidates: source claims point to the contradiction candidate (`claim A/B -> contradiction_candidate`). This keeps contradiction candidates to the right of ordinary structured objects in the horizontal layout and makes them read as objects built from two prior claims, not as ordinary peer objects in the same column. In this object-centered map, contradiction candidates do not draw their own direct source-reference legs; their source path is intentionally read through the two input claims to avoid duplicate source lines.
- In contradiction-candidate focus views, the frontend graph layer filter treats the input claims as source-carrier nodes, so the default `Irat` and `Forráshivatkozás` layers remain visible without reintroducing direct candidate-to-source edges.
- `Design_documents/32_relationship_map_related_objects_plan.md` is implemented as the current Kapcsolati térkép related-object slice: the old checkbox/layer-based, shared-source `Kapcsolódó objektumok` behavior has been removed from frontend and backend, and replaced by an explicit related-by-document workflow driven by the graph-selected object node. The dedicated `Kapcsolódó objektumok` panel sits directly after `Megjelenítendő objektum`, triggers `POST /api/v1/cases/{case_id}/graph/related-by-documents`, exposes `Keresés`, `Láthatók kijelölése`, and selected-result map insertion, then adds only user-selected related objects back to the map under the existing 50 focus-object cap. Related/object selector cards use the same graph object-type color roles for visual grouping instead of showing a separate type/shared-document metadata line.
- `Kapcsolati térkép` frontend surface is implemented and the multi-focus flow is a technically complete baseline. The sidebar has a dedicated `Kapcsolati térkép` surface, `frontend/src/api.ts` has graph response types plus `getRelationshipGraphForObjects` and `getRelationshipRelatedObjectsByDocuments`, and the surface can load a graph through the `Ügy munkapad` handoff or source-valid object-card selection built from the graph module's own source-valid object list. Object cards now use checkbox multi-selection across type-filter changes, show `x kijelölve / 50` plus an `Összes: n` chip for the complete source-valid selectable object count, and load graphs through `Térkép frissítése kijelölésből`; `Láthatók kijelölése` and `Láthatók levétele` operate on the current filtered list, and `Térkép ürítése` clears the displayed graph and related-object panel state. The object-type selector defaults to `Összes`. It uses a two-row graph workbench layout: a top inspector row ordered as `Megjelenítendő objektum`, `Kapcsolódó objektumok`, `Kijelölt csomópont tartalma`, and `Kapcsolatok`, plus a full-width React Flow / XYFlow canvas below. Node clicks update the selected-node content inspector and can activate the related-by-document search when the node is a supported structured object; edge clicks update the `Kapcsolatok` inspector. Graph editing/connecting remains disabled, but temporary mouse-drag repositioning of nodes is enabled and is not persisted. Frontend graph layer toggles are explicit and ordered as `Irat`, `Oldal`, `Szövegrész`, `Forráshivatkozás`, `Ellentmondások`; `Irat` and `Forráshivatkozás` are enabled by default. Next graph work should focus on live UX/readability tuning and later expansion decisions, not on missing core multi-focus or related-object plumbing.
- Frontend graph dependency/tooling status: `@xyflow/react` is lazy-loaded through `frontend/src/RelationshipFlowCanvas.tsx`; the main bundle no longer carries the React Flow implementation eagerly. `frontend/package.json` pins `esbuild` to `0.28.1` through npm `overrides` so the current Node 18 WSL runtime can keep Vite 6 while `npm audit --audit-level=high` stays clean. `frontend/vite.config.ts` targets `es2022` and uses a realistic local-app chunk warning limit.
- Safe early document discard/delete is fixed for PDF/text-review documents that have not yet become source material. The discard path now removes document search entries, chunk manifests, and text layers before deleting discardable analysis runs, so `Elvetés / törlés` works on active, zero-chunk documents without foreign-key failures.
...
- The `Teljes iratfeldolgozás` surface now uses a cleaner 1:1 first row: `Irat és feldolgozási profil` on the left and `Iratösszefoglaló` on the right, with `Előkészített munkalista` as a full-width second row. Profile/page-range controls align on desktop, mobile splits them into separate rows, placeholder/summary cards use the shared UI token style, and summary content no longer stretches vertically to fill the panel.
...
- Configured embedding model defaults to `text-embedding-bge-m3`. Embedding calls auto-ensure the configured embedding model is loaded through LM Studio native `/api/v1/models/load` before calling OpenAI-compatible `/v1/embeddings`. Embedding model loading uses `context_length=4096`; LM Studio currently rejects `eval_batch_size`, `flash_attention`, and `offload_kv_cache_to_gpu` for embedding models, so those are intentionally not sent for embedding load. Chat model loading currently targets `qwen/qwen3.5-9b` with `context_length=61440`, `eval_batch_size=4096`, `flash_attention=true`, and `offload_kv_cache_to_gpu=true`. The quality profile `qwen/qwen3.6-35b-a3b` remains a documented local opt-in with `eval_batch_size=512`.
...
- Current workstation profile: chat `qwen/qwen3.5-9b`, chat `context_length=61440`, chat `eval_batch_size=4096`, BGE-M3 embedding `context_length=4096`, and LLM request timeout `900` seconds for long full-document runs. Quality opt-in: `qwen/qwen3.6-35b-a3b` with `eval_batch_size=512`.
...
- `Design_documents/27_relationship_map_graph_implementation_plan.md` turns that graph-view direction into the concrete implementation plan. The source-valid graph baseline now uses the multi-focus POST endpoint, explicit node/edge types, graph projection schemas/service/API, React Flow / XYFlow frontend surface, `Ügy munkapad` handoff, source-valid checkbox object-card selector, and frontend graph layer toggles. The old UUID-shaped single-focus GET endpoint and frontend helper have been retired.
- Kapcsolati térkép object-selection cards and related-object result cards now share content-driven height behavior. The old graph-object fixed minimum height is no longer applied to these checkbox cards, so Megjelenítendő objektum and Kapcsolódó objektumok remain stable under browser/OS display scaling.
- Design_documents/34_relationship_map_canvas_selection_plan.md records the implemented relationship-map canvas object-removal slice: canvas-level single/bulk selection is limited to focus object nodes, object removal updates the focus list and reloads the graph, context/provenance nodes are intentionally non-removable, and inspector panels remain single-node/edge detail surfaces.
- `Design_documents/28_relationship_map_horizontal_layout_plan.md` captures the current visual strengthening direction for `Kapcsolati térkép`: render the graph with a deterministic horizontal layer layout (`irat` -> `oldal` -> `szövegrész` -> `forráshivatkozás` -> `fókuszobjektum` -> `kapcsolódó objektum / ellentmondás`) so the canvas reads like a provenance/workbench map instead of a scattered network.
- The horizontal graph layout helper is implemented in `frontend/src/RelationshipFlowCanvas.tsx`. The canvas now maps node types into deterministic left-to-right layers, recognizes all `focus_node_ids` for labeling/logic, places contradiction candidates to the right of ordinary objects, and sets left/right React Flow handle positions. Within each visible layer, a frontend-only crossing-aware ordering pass uses neighboring layer positions to reduce avoidable edge crossings while keeping the fixed column narrative intact. Layer checkbox changes force a fresh React Flow layout, and ordering passes with no useful neighbor information no longer reset object order back to the fallback focus/label order. The current accepted graph-node visual baseline is deliberately restrained: graph nodes share the same border/effect baseline, selected/focus state does not add extra node styling, and only structured object nodes receive subtle background color by object type. Latest frontend verification for this slice: `npm --prefix frontend run build` passed.
- When intermediate source layers are hidden in `Kapcsolati térkép`, the frontend may draw label-free dashed `VISUAL_SOURCE_BRIDGE` edges between the remaining visible source-chain nodes. These are visual-only bridge lines for readability; they are not persisted and do not represent new backend graph edges. Bridge edges are suppressed if a real visible edge already exists between the same two nodes, regardless of direction; real source-chain edges stay solid.
- `Kapcsolati térkép` inspector panels are intentionally static read-only detail surfaces. `Kijelölt csomópont tartalma` and `Kapcsolatok` cards do not change background, border, or shadow because a node/edge is selected; the selected graph element only drives what those panels display. Their cards/chips are connected to graph-inspector CSS tokens while retaining the global compact card/chip look.
- The `Kapcsolati térkép` backend source-location projection now uses the deterministic hierarchy `document -> page -> chunk -> source_reference -> object` instead of the older `source_reference -> document/page/chunk` star. Fallbacks are documented and covered for missing page/chunk data. `Elemzési eredet` has been removed from this object-centered map and is reserved only for a possible later lifecycle/provenance view.
- `Design_documents/26_review_report_status_cleanup_plan.md` records the implemented review/source status cleanup for `Áttekintési jelentés`: `new` review status is retired from active backend/frontend contracts, `corrected` is shown as `Korrekcióval kizárt`, and objects orphaned by merge or final source detachment carry the corrected/source-invalid semantics consistently. Migration `0048_review_status_cleanup` converts any historical `new` review statuses to `needs_review`, and migration `0049_contradiction_partial_pair` lets contradiction candidates keep partial/missing pairs only after correction.
...
- `Teljes iratfeldolgozás` person-search worklist cards now receive more useful source evidence: the backend expands the matched person-name span to the containing source sentence and stores that sentence as `source_evidence_json[].quote_text`. This keeps the LLM prompt compact while making the card footer show the local source context instead of repeating only the name.
- `Design_documents/29_full_document_free_question_plan.md` records the `Szabad iratkérdés` full-document slice. The first backend/frontend implementation is now in place through migration `0050_full_document_answers`: selected active document pages plus a one-line user question create persistent `full_document_answers`, separate from the person-only `document_processing_items` worklist. The UI is profile-dependent: `Személykeresési fókuszok` keeps `Előkészített munkalista`, while `Szabad iratkérdés` shows an `Iratválasz` style answer panel with safe Markdown rendering, previous-answer selection through the shared tokenized choice-button style, automatic saved-answer refresh after deletion, and active-answer deletion. The free-question parser is deliberately tolerant when `answer_text` is recoverable: malformed optional `insufficient_source`, unescaped internal quotes, and a missing final object brace no longer discard otherwise usable long answers.
...
- Manual contradiction candidate cleanup is implemented: detaching an `A` or `B` claim side from the detail panel, or deleting a claim/event used by a candidate, marks the candidate `review_status=corrected` / `Korrekcióval kizárt` instead of deleting it as an invisible side effect. This does not force `source_validation_status=source_invalid`, because the candidate's source basis is inherited through its input objects.
...
- Review report items that are `corrected` or `source_invalid` can be permanently deleted through an explicit backend/frontend action; dependent contradiction candidates are corrected when an input claim/event disappears, so no broken active references remain.
...
- BoberDetective uses dedicated local ports so it can run beside AI Assistant without collision: frontend `http://localhost:5174`, backend `http://127.0.0.1:8001`, and Vite proxies `/api` to backend port `8001`.
- Root `.env` is intentionally Git-ignored and is now loaded by the backend without overriding already exported process variables. It supplies local-only configuration such as `BOBERDETECTIVE_LLM_API_KEY` when LM Studio API authentication is enabled; the existing provider clients send that key as a Bearer token.
- After machine restart, start the complete BoberDetective stack from PowerShell with `powershell -ExecutionPolicy Bypass -File \\wsl.localhost\Ubuntu-24.04\home\bober\projects\Codex_BoberDetective\scripts\start.ps1`. It starts only BoberDetective Docker services plus its backend/frontend and verifies both endpoints. Stop it with the sibling `scripts\stop.ps1` command. Alembic should report `0051_assistant_chats (head)`.
- The PowerShell entry points delegate process handling to tracked `scripts/start-services.sh` and `scripts/stop-services.sh`; PID files and logs remain under `/tmp/boberdetective-*.pid` and `/tmp/boberdetective-*.log`.
...
research_findings, document_processing_items, full_document_answers, detached_source_items,
rag_answers,
alembic_version
```
...
3. Continue `Design_documents/27_relationship_map_graph_implementation_plan.md` and `Design_documents/28_relationship_map_horizontal_layout_plan.md` as the current larger development slice. The multi-focus backend/frontend graph baseline is implemented, including `Ügy munkapad` handoff, source-valid object-card selector, React Flow canvas, inspectors, graph layer toggles, and backend source-location hierarchy projection.
4. Next graph step: live-test the current `Kapcsolati térkép` UI with multiple selected objects and the new `Irat -> Oldal -> Szövegrész -> Forráshivatkozás -> Objektum` chain. Check layer toggles, bridge edges, graph readability, source-invalid exclusion, and whether a separate lifecycle/provenance graph is worth planning later.
...
8. The AI-asszisztens work surface is implemented and accepted as the current first UI/UX baseline from Design_documents/31_ai_assistant_chat_module_plan.md. Migration 0051_assistant_chats adds assistant_chats and assistant_messages; backend routes support chat list/create/detail/rename/soft-delete, message send, and latest-assistant-answer regeneration through the existing LM Studio provider. The module is intentionally generic and independent: no case/document/RAG/object/provenance coupling. The frontend places it after Tudásbázis and before Audit napló, with a slim conversation rail, three-dot rename/delete context menu, tokenized in-app rename dialog, stable internal chat canvas, centered empty-state composer, bottom-row composer during active conversations, Markdown-rendered assistant bubbles, typing indicator, and internal message-thread scrolling so the input does not jump with conversation length. Its delete action uses the shared tokenized app-dialog confirmation rather than a native browser popup; deleting the active chat now automatically opens the newest remaining chat, or a clean empty start state if no chat remains. New chats no longer accept create-time titles; they start with Új beszélgetés, auto-title from the first user message, and can later be renamed through PATCH. The shared popup role tokens now style both searchable-select dropdowns and assistant context menus. Latest verification: .venv/bin/python -m pytest tests/test_assistant.py tests/test_llm.py returned 28 passed, npm --prefix frontend run build passed, git diff --check passed, and Alembic is at 0051_assistant_chats (head). AI-asszisztens convenience controls are now part of the current slice: assistant answers expose copy and latest-answer-regenerate icons below the answer bubble, and the composer has a Gondolkodó toggle that sends normal or model_default reasoning mode per message/regeneration. Regeneration intentionally deletes only the latest assistant answer and asks again from the previous user message; the frontend uses an in-flight guard plus typing state so repeated clicks cannot queue parallel regenerate calls. Enter now keeps its normal textarea role as a line break; sending happens through the composer send button. The composer textarea is stabilized with an inner input shell, no height jump on first typing, a size-stable focus border, and a global tokenized scrollbar style that is reused by scrollable UI areas. Streaming is deliberately out of scope for this module for now. The assistant context-window hardening slice in Design_documents/33_ai_assistant_context_window_plan.md is implemented: assistant LLM calls now include a minimal generic system prompt, use an explicit conservative 120000-character hard budget with frontend/backend user feedback instead of silent long-history truncation, and allow a single assistant prompt up to the same 120000-character ceiling for large pasted-document workflows. The too-long prompt warning now has its own composer grid row so it cannot push the send/reasoning buttons out of place. Latest targeted AI-asszisztens/LLM verification is 28 passed. Future AI-asszisztens work should stay concrete live-issue-driven UX polish, not new investigative coupling.
...
- Current preferred chat-model LM Studio load profile: model `qwen/qwen3.5-9b`, `context_length=61440`, `eval_batch_size=4096`, `flash_attention=true`, `offload_kv_cache_to_gpu=true`, `echo_load_config=true`. The quality profile `qwen/qwen3.6-35b-a3b` remains a documented local opt-in with `eval_batch_size=512`.
...
- Frontend dev server proxies `/api` to backend port `8001`.
