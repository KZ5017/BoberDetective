...
- Frontend source-search strategy selection is available for document/case finding search. `Szovegresz plafon` defaults to 30 and is capped at 50; the same cap is enforced by the backend. `Batch meret` defaults to 5, is backend-validated between 1 and 15, and controls how the selected source chunks are split into LLM calls. Semantic/hybrid index readiness is required before semantic/hybrid retrieval can run. User-side smoke testing confirmed that short concrete focus terms may lose recall in larger batches even when retrieval selected the correct chunk, so the frontend focus helper recommends trying `1-3` for short, concrete focus values.
...
- The active `search_findings` LLM prompt now uses English instruction text while explicitly requiring Hungarian source analysis and Hungarian user-facing output fields. `quote_text` must still be copied character-exactly from the Hungarian source. A user-side A/B smoke showed a clear quality improvement on precise person-focused searches, especially avoiding identity conflation such as mixing Pauline Dubourg with Mademoiselle L'Espanaye. The prompt now generalizes the positive direct-focus rule beyond person names: if the QUERY names or clearly describes a concrete focus item and the SOURCE directly contains the same item with a factual statement, role, attribute, action, location, date/time, amount, relationship, status, reference, or observation, it should be treated as direct information about the focus. Concrete focus items include persons, organizations, locations, phone numbers, email addresses, license plates, money amounts, case references, document references, and attachments.
