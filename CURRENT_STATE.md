...
- The horizontal graph layout helper is implemented in `frontend/src/RelationshipFlowCanvas.tsx`. The canvas now maps node types into deterministic left-to-right layers, recognizes all `focus_node_ids`, places contradiction candidates to the right of ordinary objects, sets left/right React Flow handle positions, and gives document/page/chunk/source-reference/object/contradiction nodes subtly different token-based visual roles. Latest frontend verification for this slice: `npm --prefix frontend run build` passed.
