...
```text
pytest full suite, latest known: 392 passed
...
latest targeted AI-asszisztens/LLM slice: 25 passed
...
- `Kapcsolati térkép` backend graph projection is implemented through the multi-focus API. `POST /api/v1/cases/{case_id}/graph/objects` returns a read-only, deduplicated node/edge projection for 1..20 source-valid focus objects. Supported focus types are `claim`, `event`, `entity`, `missing_item_candidate`, and `contradiction_candidate`; the projection can include direct source references, document/page/chunk location nodes, contradiction claim-pair edges, shared-source neighbors, node/edge limits, and warnings. It is not a graph database and does not create AI-generated edges. `Elemzési eredet` is intentionally removed from this object-centered graph: analysis-run and research-finding lifecycle provenance remains available through analysis/audit surfaces instead of graph nodes. Chunk-bound source references without explicit `page_id` are resolved to the current document page through the page-local chunk's `page_start` when possible. The old UUID-shaped single-focus GET endpoint has been removed from the public API; service-level single-focus compatibility delegates to the multi-focus builder.
...
- `Kapcsolati térkép` frontend surface is implemented and the multi-focus flow is a technically complete baseline. The sidebar has a dedicated `Kapcsolati térkép` surface, `frontend/src/api.ts` has graph response types and `getRelationshipGraphForObjects`, and the surface can load a graph through the `Ügy munkapad` handoff or source-valid object-card selection built from the current review report. Object cards now use checkbox multi-selection across type-filter changes, show `x kijelölve / 20`, and load graphs through `Térkép frissítése kijelölésből`; `Láthatók kijelölése` and `Láthatók levétele` operate on the current filtered list, and `Térkép ürítése` clears the displayed graph. The object-type selector defaults to `Összes`. It uses a two-row graph workbench layout: a top inspector row with `Megjelenítendő objektum`, `Kijelölt csomópont tartalma`, and `Kapcsolatok` panels, plus a full-width React Flow / XYFlow canvas below. Node clicks update the selected-node content inspector, edge clicks update the `Kapcsolatok` inspector, and node selection shows direct related edges when no edge is selected. Graph editing/connecting remains disabled, but temporary mouse-drag repositioning of nodes is enabled and is not persisted. Frontend graph layer toggles are explicit and ordered as `Irat`, `Oldal`, `Szövegrész`, `Forráshivatkozás`, `Kapcsolódó objektumok`, `Ellentmondások`; `Irat` and `Forráshivatkozás` are enabled by default. Next graph work should focus on visual strength/readability, live UX tuning, and later expansion decisions, not on missing core multi-focus mechanics.
...
- The horizontal graph layout helper is implemented in `frontend/src/RelationshipFlowCanvas.tsx`. The canvas now maps node types into deterministic left-to-right layers, recognizes all `focus_node_ids` for labeling/logic, places contradiction candidates to the right of ordinary objects, and sets left/right React Flow handle positions. The current accepted graph-node visual baseline is deliberately restrained: graph nodes share the same border/effect baseline, selected/focus state does not add extra node styling, and only structured object nodes receive subtle background color by object type. Latest frontend verification for this slice: `npm --prefix frontend run build` passed.
