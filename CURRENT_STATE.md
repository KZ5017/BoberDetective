...
- The frontend visual language has been softened: global buttons are flatter with subtle color/border state changes and no hover/active movement, many document/card/panel titles and status chips use lighter font weights, and the Full HD media query is intentionally limited to small button/input/chip typography and spacing tweaks.
