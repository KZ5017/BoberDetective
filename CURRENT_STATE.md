...
- `Ügy munkapad` is now focused on analysis, research findings, review report, detached sources, and downstream work. Its semantic index readiness/status block is a fixed compact strip above the `Elemzés` panel in the left column, so the right-side `Kutatási találatok` panel can start at the top of the workbench.
- Two-column work areas in `Ügy munkapad`, `Teljes iratfeldolgozás`, and the document organizer use equal-width columns. Analysis controls have been compacted into a single settings row where possible, and source-scope helper text uses a shared subdued hint style.
...
- The `Kutatási találatok` panel now shows the latest `search_findings` run summary above the worklist: validation status, saved finding count, non-confirmed finding count, backend-validation rejected candidate count, and the first rejected-candidate reasons. Findings with an exact or backend-repaired quote are saved as source-valid research-finding worklist items; findings whose quote cannot be repaired are still saved as `source_invalid` / `unconfirmed` worklist items with warning styling and the existing `Nincs érvényes forráshivatkozás` label.
