...
- Full-document prompt strategy is intentionally compact: the system prompt keeps only source-faithfulness / no outside knowledge / no legal blame / valid JSON constraints, and the user prompt asks for named characters/items, one short Hungarian role/description, and the source page label. The model is not asked to produce character-exact quotes in this workflow.
