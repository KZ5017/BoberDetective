...
Azota a frontend import UI, az analysis source filter, a legacy `document_type` kivezetese es az audit-trackelt irat atbesorolas is elkeszult. Kapcsolodo kovetkezo nagyobb temak:

- dokumentum torles / kizaras / archivalas workflow,
- dedikalt `Audit naplo` felulet az `audit_events` tablaban levo muveletekhez, beleertve a `document_reclassified` esemenyeket es megjegyzeseiket.
