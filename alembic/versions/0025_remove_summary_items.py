...
Revision ID: 0025_remove_summary_items
Revises: 0024_research_findings_worklist
...
revision: str = "0025_remove_summary_items"
down_revision: str | None = "0024_research_findings_worklist"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def upgrade() -> None:
    op.execute("delete from human_reviews where object_type = 'summary_item'")
    op.execute("delete from export_items where object_type = 'summary_item'")
    op.execute("delete from analysis_run_outputs where output_type = 'summary_item'")
    op.execute(
        "update research_findings "
        "set conversion_status = 'ignored', target_object_type = null, target_object_id = null, updated_at = now() "
        "where target_object_type = 'summary_item'"
    )
    ...
    op.drop_constraint("ck_export_items_object_type", "export_items", type_="check")
    op.create_check_constraint(
        "ck_export_items_object_type",
        "export_items",
    ...
    op.drop_constraint("ck_research_findings_target_object_type", "research_findings", type_="check")
    op.create_check_constraint(
        "ck_research_findings_target_object_type",
        "research_findings",
        "target_object_type is null or target_object_type in "
        "('claim', 'event', 'entity', 'missing_item_candidate', 'other')",
    )
...
def downgrade() -> None:
    ...
    op.create_check_constraint(
        ...
        "analysis_run_outputs",
        "output_type in ('document', 'page', 'chunk', 'entity', 'mention', 'event', 'claim', "
        "'contradiction_candidate', 'missing_item_candidate', 'export', 'summary_item', 'source_reference', 'research_finding')",
    )
    ...
    op.create_check_constraint(
        ...
        "research_findings",
        "target_object_type is null or target_object_type in "
        "('claim', 'event', 'entity', 'missing_item_candidate', 'summary_item', 'other')",
    )
