...
from app.schemas.analysis_modules import AnalysisModuleClaim, AnalysisModuleEntity, AnalysisModuleEvent, AnalysisModuleRunRequest, AnalysisModuleRunResponse
...
def run_analysis_module(
    ...
    if module_key == "extract_entities":
        return _run_extract_entities(db, case_id, payload)
    ...
    try:
        ...
        completion = LMStudioNativeProvider(settings).chat_completion(
            ...
            [
                ...
                LLMChatMessage(role="user", content=_build_extract_entities_user_prompt(payload.query, retrieved_chunks)),
            ],
            temperature=0.1,
            max_tokens=1600,
        )
...
def _build_extract_entities_user_prompt(query: str, retrieved_chunks: list[RetrievedChunk]) -> str:
    source_blocks = []
    for retrieved in retrieved_chunks:
        source_blocks.append(
            f"{retrieved.label}:\n"
